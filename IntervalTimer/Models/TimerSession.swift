...
class TimerSession: ObservableObject {
    ...
    var globalStageIndex: Int {
        (currentRound - 1) * configuration.stages.count + currentStageIndex
    }
    ...
    init(configuration: WorkoutConfiguration) {
        ...
        self.timeRemaining = configuration.stages.first?.duration ?? 0
    }
    ...
    func start() {
        ...
        if let firstStage = configuration.stages.first {
    ...
    private func advanceToNextStage() {
        ...
        if currentStageIndex >= configuration.stages.count {
            currentStageIndex = 0
            currentRound += 1
        }
